...
BOT_TOKEN = "7581421520:AAFdbHG2v2B1eDRKv60p4qEgGep--PTShEQ"
CHAT_ID = "5646187345"
...
known_face_names = ["Family Member", "Family Member", "Family Member", "Senin","FM"]
family_images = ["family_member_1.jpg", "family_member_2.jpg", "jk.jpg", "SENIN.jpg","SAJIL.jpg"]
...
while True:
    ...
    for i, face_encoding in enumerate(face_encodings):
        ...
        if True in matches:
            ...
            # Special alert for Senin
            if name == "Senin":
                current_time = time.time()
                if current_time - last_alert_time > alert_cooldown:
                    alert_message = "Hurry up Senin arrived please open the door"
                    last_alert_time = current_time
                    filename = f"captured_images/senin_{int(current_time)}.jpg"
                    cv2.imwrite(filename, frame)
                    response = send_telegram_alert(filename, CHAT_ID, BOT_TOKEN, alert_message)
                    send_alert_to_server(alert_message, filename, "senin_arrival")
                    print("Telegram Response:", response)
        else:
        ...
        # Draw rectangle & name
        if i < len(face_locations):  # Make sure we have a face location for this encoding
